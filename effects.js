...
function spawnPowerEffect(kind,u,lat,color,owner){
 ...
 if(kind==='zenflow'){const dome=addPowerPart(mesh,cachedPowerGeometry('dome',()=>new THREE.SphereGeometry(1,24,12,0,Math.PI*2,0,Math.PI*.62)),color,0,'time-dome');dome.scale.set(8,3,8);dome.position.y=-1;addPowerInstances(mesh,color,'clock-fragments',powerLow?12:24);}
 else if(kind==='hybrid'){let ghost;if(owner?.mesh){ghost=owner.mesh.clone(true);const mat=powerShader(color,1);ghost.traverse(o=>{if(o.isMesh)o.material=mat;});mesh.add(ghost);ghost.name='phase-ghost';ghost.scale.setScalar(1.045);ghost.position.y=-1.1;}else{ghost=addPowerPart(mesh,silhouetteGeometry(),color,1,'phase-ghost');}addPowerPart(mesh,ribbonGeometry('phase-veil',.7,1.5,3,.35),color,1,'veil');}
 else if(kind==='nexus'){const man=addPowerPart(mesh,silhouetteGeometry(),color,2,'hologram-human');man.scale.setScalar(1.3);man.position.y=.1;const sheet=addPowerPart(mesh,cachedPowerGeometry('scan-sheet',()=>new THREE.PlaneGeometry(3,4,1,24)),color,2,'scan-sheet');sheet.position.z=.3;sheet.position.y=1;}
 ...
 else if(kind==='juris'){const shell=cachedPowerGeometry('shield-facet',()=>new THREE.SphereGeometry(2.3,5,3,0,Math.PI*.28,.2,Math.PI*.8));for(let i=0;i<6;i++){const facet=addPowerPart(mesh,shell,color,4,'shield-facet');facet.rotation.y=i*Math.PI/3;}}
 else if(kind==='signal'){for(let i=0;i<2;i++){const beam=addPowerPart(mesh,ribbonGeometry('sonic-'+i,.08,1+i*.3,34,.75),color,5,'sonic-beam');beam.rotation.y=Math.PI;beam.rotation.z=i*Math.PI/2;}}
 ...
 else if(kind==='collective'){addPowerInstances(mesh,'#ffd58a','fortune-shards',powerLow?14:28);const funnel=addPowerPart(mesh,ribbonGeometry('fortune-funnel',1.5,4,1,.24),color,6,'fortune-stream');funnel.rotation.x=Math.PI/2;}
 else if(kind==='aether'){addPowerInstances(mesh,'#ffe9b7','magnet-streaks',powerLow?18:36);for(let i=0;i<2;i++){const orbit=addPowerPart(mesh,ribbonGeometry('magnet-orbit',.85,4,1,.065),color,5,'magnet-orbit');orbit.rotation.x=Math.PI/2;orbit.rotation.z=i*Math.PI;}}
 else if(kind==='animus'){const body=cachedPowerGeometry('drone-body',()=>new THREE.LatheGeometry([new THREE.Vector2(0,-.55),new THREE.Vector2(.2,-.48),new THREE.Vector2(.65,-.1),new THREE.Vector2(.72,.08),new THREE.Vector2(.4,.26),new THREE.Vector2(0,.4)],16));const drone=addPowerPart(mesh,body,color,8,'sentinel-drone');drone.position.y=2.3;for(let i=0;i<2;i++){const wing=addPowerPart(mesh,ribbonGeometry('drone-wing',.3,.7,1.6,.16),color,5,'drone-wing');wing.position.y=2.3;wing.rotation.y=i*Math.PI+Math.PI/2;}addPowerInstances(mesh,color,'drone-motes',10);}
 else if(kind==='helix'){for(let i=0;i<2;i++){const strand=addPowerPart(mesh,ribbonGeometry('dna-strand',2,1.25,4,.11),i?'#eeffff':color,7,'dna-strand');strand.rotation.x=-Math.PI/2;strand.rotation.z=i*Math.PI;strand.position.y=-1;}addPowerInstances(mesh,color,'healing-petals',powerLow?16:30);}
...
function stepPowerEffects(dt){for(let i=powerEffects.length-1;i>=0;i--){const f=powerEffects[i];f.life-=dt;if(f.life<=0){removePowerEffect(f);powerEffects.splice(i,1);continue;}
 ...
 f.mesh.traverse(p=>{if(p.material?.uniforms){p.material.uniforms.time.value=clock;p.material.uniforms.fade.value=Math.min(1,f.life*2,age*5+.2);}
  if(p.isInstancedMesh){for(let n=0;n<p.count;n++){const t=n/p.count,a=t*Math.PI*2+clock*1.6;let radius=2,up=0;powerDummy.rotation.set(clock+n,clock*.7+n,0);powerDummy.scale.setScalar(.16);
   ...
   else if(f.kind==='collective'){radius=Math.max(.15,6*(1-(age*.9+t)%1));up=.3+Math.sin(a)*.5;powerDummy.scale.set(.16,.3,.16);}
   else if(f.kind==='aether'){radius=1+((1-t-clock*.35)%1+1)%1*6;up=Math.sin(a*2)*.65;powerDummy.scale.set(.09,.7,.09);powerDummy.rotation.z=-a;}

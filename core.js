...
function texBanner(text,acc='#D4A843',sub='COLLECTIVE AI'){
  ...
  g.font='800 120px "Orbitron",system-ui,sans-serif';g.fillStyle='#F5F5F5';g.textAlign='center';g.textBaseline='middle';g.fillText(text,w/2,h/2-14);
  g.font='500 34px "Rajdhani",monospace';g.fillStyle=acc;g.fillText(sub,w/2,h/2+78);
  // diamond star marks
...
function texMark(mark,acc,acc2,size=256){ // division logo mark, drawn abstractly
  ...
  switch(mark){
    ...
    case 'crest':{g.beginPath();g.moveTo(cx,cy-R);g.lineTo(cx+R,cy-R*.5);g.lineTo(cx+R*.8,cy+R*.4);g.lineTo(cx,cy+R);g.lineTo(cx-R*.8,cy+R*.4);g.lineTo(cx-R,cy-R*.5);g.closePath();g.fill();g.fillStyle=acc2;g.font=`800 ${s*.34}px "Orbitron",sans-serif`;g.textAlign='center';g.textBaseline='middle';g.fillText('HL',cx,cy+s*.02);break;}
    ...
    case 'hex':{g.beginPath();for(let i=0;i<6;i++){const a=i/6*Math.PI*2+Math.PI/6;const x=cx+Math.cos(a)*R,y=cy+Math.sin(a)*R;i?g.lineTo(x,y):g.moveTo(x,y);}g.closePath();g.stroke();g.fillStyle=acc2;g.font=`700 ${s*.22}px "Rajdhani",monospace`;g.textAlign='center';g.textBaseline='middle';g.fillText('</>',cx,cy+s*.01);break;}
...
function buildTextures(){
  if(typeof buildSurfaceTextures==='function')buildSurfaceTextures();

...
const builds=[];racers[1].isPlayer=true;racers[1].build=['Motor'];racers[2].build=['Armor'];
const c={THREE,scene,saved:{appearance:{'1':'satin'}},applyKartBuildVisuals:(kart,build,appearance)=>builds.push({id:kart.name,build,appearance}),game:{state:'results',racers},camera:new THREE.PerspectiveCamera(),sun:new THREE.DirectionalLight(),innerWidth:1400,innerHeight:900,
...
assert.deepEqual(builds.slice(0,2),[{id:'1',build:['Motor'],appearance:'satin'},{id:'2',build:['Armor'],appearance:'factory'}],'podium retains raced builds and player finish');

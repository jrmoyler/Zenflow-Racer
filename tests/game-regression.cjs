...
const context={console,Math,Set,Map,THREE:{Vector3:Vec},window:{},document:{getElementById:element,querySelectorAll:()=>[],addEventListener:(n,fn)=>{listeners[n]=fn;},hidden:false},navigator:{getGamepads:()=>[]},localStorage:{getItem:()=>null,setItem:noop},performance:{now:()=>100},matchMedia:()=>({matches:false,addEventListener:noop}),addEventListener:(n,fn)=>{listeners[n]=fn;},requestAnimationFrame:noop,setTimeout:()=>1,clearTimeout:noop,innerWidth:1400,innerHeight:900,devicePixelRatio:1,scene:{add:noop,remove:noop},rng:()=>.99,lerp:(a,b,t)=>a+(b-a)*t,clamp:(v,a,b)=>Math.max(a,Math.min(b,v)),wrap01:v=>((v%1)+1)%1,track:{len:1000},TRACK_W:16,trackAG:()=>0,trackCurv:()=>0,orientOnTrack:noop,trackPoint:noop,trackTan:(u,v)=>v.set(0,0,1),trackUp:noop,trackRight:(u,v)=>v.set(1,0,0),_p:new Vec(),_v1:new Vec(),_v2:new Vec(),_v3:new Vec(),noiseHit:noop,SFX:new Proxy({},{get:()=>noop}),AUDIO:{on:false},audioInit:noop,audioUpdate:noop,ROSTER:roster,STAT_NAMES:[],itemBoxes:[],tokens:[],mines:[],missiles:[],sparksBlue:{emit:noop},sparksOrange:{emit:noop},sparksPink:{emit:noop},boostFx:{emit:noop},smokeFx:{emit:noop},goldFx:{emit:noop},hitFx:{emit:noop},ordinal:n=>n===1?'st':n===2?'nd':n===3?'rd':'th',renderer:{render:noop},camera:{},world:{traverse:noop},ITEMS:{burst:{name:'SIGNAL BURST'},shield:{name:'AEGIS SHIELD'},mine:{name:'LOOM MINE'},missile:{name:'VECTOR MISSILE'},pulse:{name:'OVERSEER PULSE'},triple:{name:'NODE CLUSTER'}},itemIconSVG:()=>''};
...
test('Sentinel Drone respects shields, phase, and regeneration',()=>{for(const defense of ['shield','phase','regen']){racer('animus');opponent();run(`o.${defense}=4;useSpecial(r);stepAbilities(.1)`);assert.equal(run('o.speed'),40,defense);assert.equal(run('o.slow'),0,defense);}});
...
test('AI speed obeys the same legal build for every difficulty and race gap',()=>{
 racer();opponent('signal',-.5);
 for(const diff of [0,1,2])for(const gap of [-2,0,4]){run(`game.diff=${diff};o.progress=${gap};r.progress=2;stepAI(o,1/120)`);assert.equal(run('o.rubber'),1);}
 run('game.diff=1');});
...
test('Lost touch capture cancels queued powers but ordinary quick taps still fire',()=>{
 const ev=(pointerId,type)=>({pointerId,type,preventDefault:noop});
 for(const [id,key] of [['tI','item'],['tP','special'],['tA','addon']]){
  for(const reason of ['pointercancel','lostpointercapture']){
   racer();run('resetInput()');element(id).events.pointerdown(ev(900,'pointerdown'));
   assert.equal(run(`input.${key}Edge`),true);element(id).events[reason](ev(900,reason));
   assert.equal(run(`input.${key}||input.${key}Edge`),false,`${key}: ${reason} cannot queue a cast`);
  }
  racer();run('resetInput()');element(id).events.pointerdown(ev(901,'pointerdown'));
  element(id).events.pointerup(ev(901,'pointerup'));element(id).events.lostpointercapture(ev(901,'lostpointercapture'));
  assert.equal(run(`input.${key}`),false);assert.equal(run(`input.${key}Edge`),true,`${key}: completed quick tap still casts`);
 }
});
...
test('RB/R1 fires add-on independently while LB/L1 holds drift',()=>{
 racer();run('resetInput()');const buttons=Array.from({length:16},()=>({pressed:false}));buttons[5].pressed=true;
 context.navigator.getGamepads=()=>[{connected:true,axes:[0],buttons}];run('pollGamepad()');
 assert.equal(run('input.addonEdge'),true);assert.equal(run('input.drift'),false);
 buttons[4].pressed=true;run('input.addonEdge=false;pollGamepad()');assert.equal(run('input.drift'),true);assert.equal(run('input.addonEdge'),false);
 buttons[5].pressed=false;run('pollGamepad()');buttons[5].pressed=true;run('pollGamepad()');assert.equal(run('input.addonEdge'),true);
 assert.equal(run('game.gamepadConnected'),true);context.navigator.getGamepads=()=>[];run('pollGamepad()');assert.equal(run('game.gamepadConnected'),false);run('resetInput()');
});

...
(async()=>{
 ...
 for(const required of ['maps.js','item-art.js','abilities.js','effects.js','postfx.js','menu.js','vendor/three.min.js'])assert.ok(resources.includes(required),`Entrypoint must load ${required}`);
 const scripts=[...index.matchAll(/<script\b[^>]*src="([^"]+)"/g)].map(m=>m[1]);
 for(const [before,after] of [['maps.js','world.js'],['vehicles.js','item-art.js'],['item-art.js','game.js'],['vendor/three.min.js','world.js'],['fallback-renderer.js','world.js'],['effects.js','abilities.js'],['abilities.js','game.js'],['vehicles.js','game.js'],['vendor/anime.umd.min.js','menu.js']])assert.ok(scripts.indexOf(before)<scripts.indexOf(after),`${before} must load before ${after}`);

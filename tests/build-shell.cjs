...
(async()=>{
 ...
 for(const required of ['surface-detail.js','item-models.js','reference-polish.css','maps.js','kart-materials.js','kart-clips.js','racefx.js','item-art.js','abilities.js','effects.js','postfx.js','menu.js','vendor/three.min.js'])assert.ok(resources.includes(required),`Entrypoint must load ${required}`);
 const scripts=[...index.matchAll(/<script\b[^>]*src="([^"]+)"/g)].map(m=>m[1]);
 for(const [before,after] of [['surface-detail.js','world.js'],['vehicles.js','item-models.js'],['item-models.js','game.js'],['maps.js','world.js'],['kart-materials.js','vehicles.js'],['kart-clips.js','vehicles.js'],['vehicles.js','racefx.js'],['racefx.js','game.js'],['vehicles.js','item-art.js'],['item-art.js','game.js'],['vendor/three.min.js','world.js'],['fallback-renderer.js','world.js'],['effects.js','abilities.js'],['abilities.js','game.js'],['vehicles.js','game.js'],['vendor/anime.umd.min.js','menu.js']])assert.ok(scripts.indexOf(before)<scripts.indexOf(after),`${before} must load before ${after}`);
 ...
 for(const file of ['assets/art/map-cherry.webp','assets/art/map-stormforge.webp','assets/art/map-canopy.webp','assets/art/title-screen.webp']){if(fs.existsSync(path.join(out,file)))assert.ok(cached.has(new URL(file,origin+'/').href));}

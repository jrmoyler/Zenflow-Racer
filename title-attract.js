...
function ensureTitleAttractPack(){
 if(titleAttract.karts.length)return;
...
// Drives the attract pack along the live road. Shared by the title loop and the opening film.
function stepTitleAttractPack(dt){
 ensureTitleAttractPack();
 const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches,step=reduced?0:dt;
 titleAttract.time+=step;titleAttract.u=wrap01(titleAttract.u+step*13/track.len);
 ...
 game.time+=step;updateMapScenery(step);return step;
}
// Camera pose for the attract loop; the opening film settles onto exactly this framing.
function titleAttractPose(out){
 out=out||{pos:new THREE.Vector3(),look:new THREE.Vector3(),up:new THREE.Vector3(),fov:50};
 ...
 out.pos.copy(p).addScaledVector(forward,portrait?9:8.2+orbit).addScaledVector(right,portrait?6:10).addScaledVector(up,portrait?5.8:4.1);
 out.look.copy(p).addScaledVector(right,portrait?0:6).addScaledVector(up,portrait?-2.1:.6);
 out.up.copy(up);out.fov=portrait?58:50;return out;
}
function tickTitleAttract(dt){
 ...
 stepTitleAttractPack(dt);
 const pose=titleAttractPose(),blend=titleAttract.started?1-Math.exp(-dt*3):1;
 camera.position.lerp(pose.pos,blend);camera.up.lerp(pose.up,blend).normalize();camera.lookAt(pose.look);camera.fov=pose.fov;camera.updateProjectionMatrix();titleAttract.started=true;
 const p=trackPoint(titleAttract.u,0,0,new THREE.Vector3());sun.target.position.copy(p);sun.position.copy(p).addScaledVector(SOLAR_DIRECTION,180);
 return true;
}
...
function buildFinishCeremony(){
 ...
 top.forEach((r,i)=>{
  ...
  const kart=buildKart(r.div,r.tier||'factory');if(typeof applyKartBuildVisuals==='function')applyKartBuildVisuals(kart,r.build||[],r.isPlayer?saved.appearance?.[r.div.id]||'factory':'factory');mount.add(kart);finishCeremony.karts.push(kart);
 });
 root.updateMatrixWorld(true);
}
// Podium framing shared by the results hold and the finish film's final settle.
function finishCeremonyPose(out){
 out=out||{pos:new THREE.Vector3(),look:new THREE.Vector3(),up:new THREE.Vector3(),fov:48};
 ...
 out.pos.copy(root.localToWorld(new THREE.Vector3(portrait?2:1.5+phase,portrait?8:7.5,portrait?-23:-23)));
 out.look.copy(root.localToWorld(new THREE.Vector3(portrait?0:-5.5,portrait?-3:1.3,0)));
 out.up.set(0,1,0).transformDirection(root.matrixWorld);out.fov=portrait?53:48;return out;
}
function animateFinishCeremony(dt){
 const reduced=matchMedia('(prefers-reduced-motion: reduce)').matches,step=reduced?0:dt;
 finishCeremony.time+=step;
 for(const kart of finishCeremony.karts)animateShowroomKart(kart,finishCeremony.time,step,0);
 const center=finishCeremony.root.getWorldPosition(new THREE.Vector3());sun.target.position.copy(center);sun.position.copy(center).addScaledVector(SOLAR_DIRECTION,180);
 updateMapScenery(step);return step;
}
function tickFinishCeremony(dt){
 if(game.state!=='results'){if(finishCeremony.root)clearFinishCeremony();return false;}
 // The finish film takes its first frame immediately, so the static results framing never flashes first.
 if(!finishCeremony.root){buildFinishCeremony();if(typeof playCinematic==='function'&&playCinematic('podium')&&tickCinematic(0))return true;}
 animateFinishCeremony(dt);
 const pose=finishCeremonyPose();
 camera.position.copy(pose.pos);camera.up.copy(pose.up);camera.lookAt(pose.look);camera.fov=pose.fov;camera.updateProjectionMatrix();
 return true;
}

// Follow a gentle section of the chosen road instead of orbiting a fixed world origin.
function rosterCameraPose(out){
 out=out||{pos:new THREE.Vector3(),look:new THREE.Vector3(),up:new THREE.Vector3(),fov:57};
 ...
 out.pos.copy(p).addScaledVector(f,-24).addScaledVector(right,17).addScaledVector(up,10);
 out.look.copy(p).addScaledVector(f,30).addScaledVector(up,5);out.up.copy(up);out.fov=57;return out;
}
function frameRosterCircuit(dt){
 rosterAngle+=dt*.045;
 const pose=rosterCameraPose();
 camera.position.copy(pose.pos);camera.up.copy(pose.up);camera.lookAt(pose.look);camera.fov=pose.fov;camera.updateProjectionMatrix();
 const p=trackPoint(.065,0,0,new THREE.Vector3());sun.target.position.copy(p);sun.position.copy(p).addScaledVector(SOLAR_DIRECTION,180);
}

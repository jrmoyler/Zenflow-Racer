...
function tickTitleAttract(dt){
 ...
 camera.position.copy(p).addScaledVector(forward,8).addScaledVector(right,9).addScaledVector(up,4.5);camera.up.copy(up);camera.lookAt(p.clone().addScaledVector(right,-2));camera.fov=52;camera.updateProjectionMatrix();

...
for(const map of run('MAPS'))for(const difficulty of [0,1,2]){
 ...
 run("game.racers=[];game.diff=difficulty;game.state='race';game.raceTime=0;for(let i=0;i<12;i++){const racer=new Racer(ROSTER[(i+difficulty*4)%20],false,i);racer.specialAI=Infinity;game.racers.push(racer);}game.player=game.racers[0];for(let tick=0;tick<120*360&&game.racers.some(r=>!r.finished);tick++){game.raceTime+=1/120;for(const r of game.racers){stepAI(r,1/120);stepRacer(r,1/120);}stepWorld(1/120);}");

...
for kart_id in ids:
    ...
    for angle,pos in [('front',(-5.6,7,4.5))]+([] if front_only else [('side',(8,0,3.5)),('rear',(-5.6,-7,4.5))]):
        camera.location=pos;camera.rotation_euler=(Vector((0,0,.85 if unoccupied else 1.1))-camera.location).to_track_quat('-Z','Y').to_euler()
        path=out/(kart_id+'-'+angle+'.png');scene.render.filepath=str(path)
        started=time.time();bpy.ops.render.render(write_still=True)
        records.append({'id':kart_id,'angle':angle,'file':path.name,'source':str(source/(kart_id+'.glb')),'sourceSha256':hashlib.sha256((source/(kart_id+'.glb')).read_bytes()).hexdigest(),'seconds':round(time.time()-started,2),'engine':'Blender Cycles CPU','samples':32,'cameraPosition':list(pos),'cameraTarget':[0,0,.85 if unoccupied else 1.1],'orthographicScale':camera.data.ortho_scale,'unoccupied':unoccupied,'referenceStudio':reference_studio})

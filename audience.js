...
function addAudienceSeat(base,x,y,z,side,color,seed,standing=false){
 // Stature (+-7%), tempo and a small yaw toward the action vary per spectator, deterministically.
 const stature=.93+((seed*37)%13)/13*.14,yaw=(((seed*53)%11)/11-.5)*.35;
 const m=new THREE.Matrix4().makeRotationY((side>0?Math.PI/2:-Math.PI/2)+yaw).scale(new THREE.Vector3(stature,stature,stature));
 m.setPosition(x,y,z);m.premultiply(base);
 AUDIENCE.seats.push({matrix:m,color,seed,standing,tempo:.88+((seed*29)%7)/7*.26,clip:['wave','clap','watch'][seed%3]});
}
function buildAudience(){
 ...
 const skin=[0xe2b293,0xb77955,0x704832,0xc99470,0x8d5a3b,0xf0c7a8],hair=[0x211c19,0x4d3022,0xb18c51,0x706a66,0x8a4a2a,0xd8d2c8],legs=[0x27313c,0x3a3d44,0x4b4036,0x2c3b52,0x5d6068,0x6b5a45];
 for(const part of AUDIENCE_DATA.parts){
  ...
  s.seats.forEach((seat,i)=>mesh.setColorAt(i,new THREE.Color(part.kind==='shirt'?seat.color:part.kind==='skin'?skin[(seat.seed*7)%6]:part.kind==='hair'?hair[(seat.seed*5)%6]:part.kind==='detail'?0x211c19:legs[(seat.seed*11)%6]).convertSRGBToLinear()));
...
function updateAudience(dt,force=false){
 ...
 for(let n=0;n<s.seats.length;n++){
  const seat=s.seats[n],clip=AUDIENCE_DATA.clips[seat.clip],phase=((s.time*(seat.tempo||1)+seat.seed*.173)%AUDIENCE_DATA.duration)/AUDIENCE_DATA.duration*(clip.length-1),a=Math.floor(phase),b=Math.min(a+1,clip.length-1),mix=phase-a;

...
function createGaragePreview(host){
 let gpu=null,view=null,camera=null,kart=null,environment=null,canvas=null,frame=0,observer=null,last=0,time=0,yaw=.35,signature='',drag=null,lights=[],software=false;
 ...
 function draw(){if(!gpu||!view||!camera||document.hidden)return;
  ...
  view.environment=environment?.texture||null;
  if(software){const ctx=gpu.ctx;ctx.fillStyle='#101824';ctx.fillRect(0,0,canvas.width,canvas.height);gpu.drawScene(view,camera,{x:0,y:0,width:gpu.width,height:gpu.height},false);}else gpu.render(view,camera);
 }
 ...
 function tick(now){if(!gpu)return;if(document.hidden){last=now;frame=requestAnimationFrame(tick);return;}if(software&&last&&now-last<100){frame=requestAnimationFrame(tick);return;}const dt=Math.min(software?.25:.04,(now-(last||now))/1000);last=now;time+=dt;
 ...
 function dispose(){cancelAnimationFrame(frame);frame=0;observer?.disconnect();observer=null;drag=null;
  ...
  if(canvas){canvas.remove();canvas=null;}old?.dispose?.();old?.forceContextLoss?.();software=false;
 }
 function open(){if(gpu)return true;if(typeof THREE==='undefined'||typeof buildKart!=='function'){if(note)note.textContent='3D preview is unavailable. Your garage controls remain ready.';return false;}
  try{canvas=document.createElement('canvas');canvas.className='garage-preview-canvas';canvas.tabIndex=0;canvas.setAttribute('aria-label','3D kart preview. Drag or use left and right arrow keys to rotate.');host.prepend(canvas);
   try{gpu=new THREE.WebGLRenderer({canvas,antialias:true,alpha:false});}catch(error){
    if(typeof CanvasRaceRenderer==='undefined')throw error;
    // A failed WebGL allocation may lock a canvas to that context type.
    const replacement=canvas.cloneNode(false);canvas.replaceWith(replacement);canvas=replacement;
    gpu=new CanvasRaceRenderer({canvas});if(!gpu.ctx)throw error;software=true;
   }
   ...
   gpu.setPixelRatio(Math.min(window.devicePixelRatio||1,1.5));gpu.outputEncoding=THREE.sRGBEncoding;gpu.toneMapping=THREE.ACESFilmicToneMapping;gpu.toneMappingExposure=typeof renderer!=='undefined'?renderer.toneMappingExposure:.82;
   ...
   if(!software&&typeof createSurfaceEnvironmentTarget==='function'){environment=createSurfaceEnvironmentTarget(gpu,typeof activeMap!=='undefined'?activeMap:{skyHorizon:0x73899f,skyTop:0x233747});view.environment=environment?.texture||null;}
